...
const Vector3<float>& coneBase1, const Vector3<float>& coneBase2, const Vector3<float>& coneBase3, float& intensity, float& specularIntensity)
{
  ...
  specularIntensity += pow(clamp(NdotH), gloss / 2) * shadow;
}
...
void BowlWatchy::drawTime()
{
  ...
  Vector3<float> minuteLight = {0.0f,-1.0f,0.75f};
  ...
  Vector3<float> hourLight = {0.0f,-0.5f,0.6f};
  ...
  for (int y = 0; y < 200; y++)
  {
    for (int x = 0; x < 200; x++)
    {
      ...
      bool white = (intensity * Diffuse[x + 200 * y] + specularIntensity * 255) * 0.5f > BlueNoise200[y * 200 + x];
